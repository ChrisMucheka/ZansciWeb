...
if (! empty($_POST["register-user"])) 
{
    ...
    $memberCount = $member->isMemberExists($username);
    if ($memberCount==1)     
   {        
         $errorMessage = $member->validateLoginMember( $username, $password);
        ...
        {
            header('Location: tutor_home.php?error='.$errorMessage);
        }     
    } 
    else 
        {
            $errorMessage[] = "User doesn't exists. ".$username."-".md5($password);
        }
}
?>
<!DOCTYPE html>
    ...
    ?> 
     <!-- Email -->
      ...
      <div class="d-flex justify-content-around">
        ...
        <div>
          <!-- Forgot password -->
          <a href="" style="color:#c00000">Forgot password?</a>
        </div>
      </div>
      <!-- Sign in button -->
      <!--button class="btn btn-outline-info btn-rounded btn-block my-4 waves-effect z-depth-0" type="submit">Sign in</button-->      
<button class="btn px-2 btn-info btn-block" type="submit" name="register-user" value="Login">Sign in</button>
      <!-- Register -->
      <p>Not a member?
        <a href="student-signup.php" style="color:#c00000">Register</a>
      </p>
      <!-- Social login -->
      <p>or sign in with:</p>
