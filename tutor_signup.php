...
if (! empty($_POST["register-user"])) {
    ...
    $errorMessage = $member->validateMember( $password, $username);
    ...
    if (empty($errorMessage)) 
    {
        ...
        if ($memberCount == 0) 
        {
            $insertId = $member->insertMemberRecord( $password, $email,$username);
            echo $insertId;
            $_SESSION[username]=$username;
             header('Location: tutor_home.php?insertId='.$insertId);
            if (!empty($insertId)) 
            {
...
?>
<div class="md-form">
        <input type="text" name="username" id="materialLoginFormName" class="form-control">
...
>Register</button>
      ...
      <p>Already a registered member?
        <a href="tutor_signin.php" style="color:#c00000">Login</a>
      or sign in with:</p>
